...
async def upload_xray(file: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            ...
            print(f"[API] X-ray analyzer error: {e}")
            xray_result = {
                "opacity_score": 0.0,
                ...
                "findings": [{"feature": "Error", "value": 0, "severity": "error",
                              "message": f"Analysis failed: {str(e)}"}],
                "confidence": 0,
                "tb_likelihood": 0.0,
            }

        # FIX: Do NOT divide by 100 — scores are already in [0, 1]
        opacity = xray_result.get("opacity_score", 0.0)
        cavity = xray_result.get("cavity_probability", 0.0)
        nodule = xray_result.get("nodule_density", 0.0)
        pleural = xray_result.get("pleural_thickening", 0.0)

        # Run QML with X-ray features integrated
        patient_with_xray = base_patient.copy()
        patient_with_xray[12] = opacity   # FIX: No /100 division
        ...
        # Gemini AI Report
        gemini_report = await _generate_gemini_report(xray_result, image_bytes)

        # Advisory
        advisory = []
        for finding in xray_result.get("findings", []):
            advisory.append({
                "severity": finding.get("severity", "info"),
                "title": finding.get("feature", "Finding"),
...
async def _generate_gemini_report(xray_result: dict, image_bytes: bytes) -> str:
    """Generate AI diagnostic report using Gemini."""
    ...
    try:
        ...
        opacity = xray_result.get("opacity_score", 0)
        cavity = xray_result.get("cavity_probability", 0)
        nodule = xray_result.get("nodule_density", 0)
        pleural = xray_result.get("pleural_thickening", 0)
        method = xray_result.get("analysis_method", "unknown")
        tb_likelihood = xray_result.get("tb_likelihood", 0)

        # FIX: Properly format percentages in prompt
        prompt = f"""You are a senior pulmonologist reviewing a chest X-ray for TB screening.

Analysis method: {method}
Extracted radiological features (0-100% scale):
- Lung Opacity Score: {opacity * 100:.1f}%
- Cavity Probability: {cavity * 100:.1f}%
- Nodular Density: {nodule * 100:.1f}%
- Pleural Thickening: {pleural * 100:.1f}%
- Computed TB Likelihood: {tb_likelihood:.1f}%

Provide a concise 2-3 sentence diagnostic impression. Be honest — if scores are low, say no significant findings. If elevated, mention differential diagnoses. Always recommend clinical correlation."""
